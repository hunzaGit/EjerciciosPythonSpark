...
# ********************************************************************************
# in (Tres tipos distintos de lineas):
...
# ********************************************************************************
# **********************  Funcion de filtrafo de meteorito  **********************
# ********************************************************************************

# in: (tipoOClase, masa, anyo) 
def filtrarMeteoritos(tupla):
	if paramTipoMeteo != None and tupla != None: # Si se busca por tipo
		return tupla[0] == paramTipoMeteo
	elif paramClaseMeteo != None and tupla != None:   # Si se busca por Clase
		return tupla[0] == paramClaseMeteo
	else:
		return False
# out: (tipoOClase, masa, anyo)
# ********************************************************************************
...
# [[line],[line],...]  -- Lineas sin comillas
# ['2008-12-31','151.117126','154.495163','150.327271','152.830978','152.830978','5811000']

lines = lines.map(formatearFila)
# (tipoOClase, masa, anyo) 

linesFilter = lines.filter(filtrarMeteoritos)
# (tipoOClase, masa, anyo)


# Calcular masa total de cada anyo
# --------------------------------
anyoMasa = linesFilter.map(lambda tupla: (tupla[2],float(tupla[1])))
# (anyo, masa)

linesMasaAcum = anyoMasa.reduceByKey(lambda acum,n: acum+n)
# ('anyo',masaTotal) -- Masa total por año
# --------------------------------


# Calcular el numero de impactos de cada anyo
# -------------------------------------------
impactoAnyo = linesFilter.map(lambda tupla: (tupla[2],1))
# ('anyo',1) -- Media diaria 

impactoAnyoAcum = impactoAnyo.reduceByKey(lambda acum,n: acum+n)
# ('anyo',numImpact) -- Numero de impactos por anyo
# -------------------------------------------

joinRDD = linesMasaAcum.join(impactoAnyoAcum)
# ('anyo', (masaTotal, numImpact)
